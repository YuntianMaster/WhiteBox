...
void UBT_Enemy_Patrol::RandomPatrol(UBehaviorTreeComponent& OwnerComp)
{
	...
	if (NavSys)
	{
		...
		PatrolPoint = NewLocation.Location;
	}
	//UE_LOG(LogTemp, Warning, TEXT("Move %s"), *playerloc.ToString())
	//UKismetSystemLibrary::DrawDebugSphere(GetWorld(), PatrolPoint, 20.f, 12, FColor::Red, 10.f, 5.f);
	FAIMoveRequest MoveRequest{ PatrolPoint };
	MoveRequest.SetUsePathfinding(true);
	MoveRequest.SetAcceptanceRadius(AcceptRadiu);
	if (FVector::Dist(playerloc, PatrolPoint) <= AcceptRadiu)
	{
		ArrivedHandle();
		return;
	}
...
void UBT_Enemy_Patrol::LinePartrol(UBehaviorTreeComponent& OwnerComp)
{
	...
	isArrived = false;
	APawn* Pawn = controllerRef->GetPawn();
	const FVector& TargetPoint = PatrolPoints[PointIndex];
	if (Pawn && FVector::Dist(Pawn->GetActorLocation(), TargetPoint) <= AcceptRadiu)
	{
		ArrivedHandle();
		return;
	}
	FAIMoveRequest MoveRequest{ TargetPoint };
	MoveRequest.SetUsePathfinding(true);
	MoveRequest.SetAcceptanceRadius(AcceptRadiu);
	controllerRef->ReceiveMoveCompleted.AddUnique(MoveDelegate);
	controllerRef->MoveTo(MoveRequest);
...
void UBT_Enemy_Patrol::ArrivedHandle()
{
	isArrived = true;
	PointIndex++;
	PointIndex = UKismetMathLibrary::Wrap(PointIndex, -1, PatrolPoints.Num() - 1);
}
